...
def inference(input_tensor,batch_size,regularizer,keep_pro):
    ...
    with tf.name_scope("layer2-pool1"):
        pool1 = tf.nn.max_pool(relu1, ksize = [1,2,2,1],strides=[1,2,2,1],padding="VALID",name='pooling1')

    with tf.variable_scope("layer3-conv2"):
        conv2_weights = tf.get_variable("weight",[5,5,32,64],initializer=tf.truncated_normal_initializer(stddev=0.1))
        variable_summaries(conv2_weights)
        conv2_biases = tf.get_variable("bias", [64], initializer=tf.constant_initializer(0.1))
        variable_summaries(conv2_biases)
        conv2 = tf.nn.conv2d(pool1, conv2_weights, strides=[1, 1, 1, 1], padding='SAME')
        relu2 = tf.nn.relu(tf.nn.bias_add(conv2, conv2_biases))

    with tf.name_scope("layer4-pool2"):
        pool2 = tf.nn.max_pool(relu2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID',name='pooling2')

    with tf.variable_scope("layer5-conv3"):
        conv3_weights = tf.get_variable("weight",
                                        [3,3,64,128],
                                        initializer=tf.truncated_normal_initializer(stddev=0.1))
        conv3_biases = tf.get_variable("bias", [128], initializer=tf.constant_initializer(0.1))
    ...
    with tf.variable_scope("layer6-conv4"):
        conv4_weights = tf.get_variable("weight",
                                        [3,3,128,256],
                                        initializer=tf.truncated_normal_initializer(stddev=0.1))
        conv4_biases = tf.get_variable("bias", [256], initializer=tf.constant_initializer(0.1))
    ...
    with tf.variable_scope("layer7-conv5"):
        conv5_weights = tf.get_variable("weight",
                                        [3,3,256,512],
                                        initializer=tf.truncated_normal_initializer(stddev=0.1))
        conv5_biases = tf.get_variable("bias", [512], initializer=tf.constant_initializer(0.1))
    ...
    with tf.name_scope("layer8-pool3"):
        pool3 = tf.nn.max_pool(drop5, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID',name='pooling3')
    ...
    with tf.variable_scope('layer11-softmax'):
